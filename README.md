...
Context API helps to solve what is known as 'prop drilling' or passing props from the top down into components that need the statly information.

- [ ] In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?

Actions - are payloads of information that send data from your application to your store. They are the only source of information for the store. Reducer - is a function that determines changes to an application's state. It uses the action it receives to determine this change.
Store - where the state for the application lives and is updated, accessed for information, and then passed to components that require that information. The Store is known as the single source of truth because there is only one place which represents the state of the application and the UI listens to it. This means that the only way to change you data in UI is to dispatch a redux action which will change the state within the redeux reducer

- [ ] What is the difference between Application state and Component state? When would be a good time to use one over the other? 

Generally component state is stored locally within a component and is not accessible from other components unless it's explicitly passed as props to it's sub components. Whereas your application state is global, and can be passed directly to the component that needs it without prop drilling.
...
My favorite state management system is context API. I was really comfortable with passing props and context API is very similar (in my eyes) which makes it easier for me to understand and traverse in my applications.
